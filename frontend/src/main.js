    ...
    try {
        ...
        eventsList.innerHTML = `
            <div style="display: flex; gap: 10px;">
                <input type="text" id="searchField" placeholder="Search" class="mdl-textfield__input search"/>
                <input type="text" id="alertSearchField" placeholder="Alert Keyword" class="mdl-textfield__input search_alert"/>
            </div>
        `;
        ...
        if (events.length > 0) {
                    ...
                    ${events.map(event => {
                        ...
                        return `
                            <tr>
                                ...
                                <td class="mdl-data-table__cell--non-numeric keywords">${keywordsString}</td>
                            </tr>
                        `;
                    }).join('')}
                </tbody>
            ...
            const alertSearchField = document.getElementById('alertSearchField');

            // Initialize List.js only for the search field
            var options = {
            ...
            // Handle alert search functionality manually
            alertSearchField.addEventListener('input', () => {
                const searchTerm = alertSearchField.value.toLowerCase();
                document.querySelectorAll('td.mdl-data-table__cell--non-numeric').forEach(cell => {
